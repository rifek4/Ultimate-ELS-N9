...
#define CPKT_LOG_EAP_IDENTITY		1
#define CPKT_LOG_EAP_SUCCESS		3
...
#define CPKT_LOG_SNAP_HDR_LEN	6	/* 802.3 LLC/SNAP header length */
static const uint8 dhd_cpkt_log_llc_snap_hdr[] = {0xaa, 0xaa, 0x03, 0x00, 0x00, 0x00};
...
dhd_pktlog_ring_add_pkts(dhd_pub_t *dhdp, void *pkt, uint32 pktid, uint32 direction)
{
	...
	pktlog_ring->pktcount++;
	spin_unlock_irqrestore(&pktlog_ring->pktlog_ring_lock, flags);
	return BCME_OK;
}
...
{
	osl_rb_node_t *n = pktlog->cpkt_log_tt_rbt.rb_node;
	...
	while (n) {
		node = CONTAINEROF(n, dhd_cpkt_log_ts_node_t, rb);
...
dhd_cpkt_log_get_802_1x_subtype(eapol_header_t *eapol)
{
	...
	if (eapol->type != EAPOL_KEY) {
		eap = (eap_header_fmt_t *)eapol->body;

		switch (eap->type) {
		case CPKT_LOG_EAP_IDENTITY:
		...
		if (eap->code == CPKT_LOG_EAP_SUCCESS)
...
static int
dhd_cpkt_log_get_pkt_ether_type(osl_t *osh, void *pktbuf,
	uint8 **data_ptr, int *len_ptr, uint16 *et_ptr, bool *snap_ptr)
{
	...
	if (length < ETHER_HDR_LEN) {
		DHD_ERROR(("BCM: %s: short eth frame (%d)\n",
		           __FUNCTION__, length));
		return BCME_ERROR;
	} else if (ntoh16_ua(frame + ETHER_TYPE_OFFSET) >= ETHER_TYPE_MIN) {
		/* Frame is Ethernet II */
		pt = frame + ETHER_TYPE_OFFSET;
	} else if (length >= ETHER_HDR_LEN + CPKT_LOG_SNAP_HDR_LEN + ETHER_TYPE_LEN &&
	           !bcmp(dhd_cpkt_log_llc_snap_hdr, frame + ETHER_HDR_LEN, CPKT_LOG_SNAP_HDR_LEN)) {
		pt = frame + ETHER_HDR_LEN + CPKT_LOG_SNAP_HDR_LEN;
		snap = TRUE;
	} else {
		DHD_ERROR((" %s: non-SNAP 802.3 frame\n", __FUNCTION__));
		return BCME_ERROR;
	}
	...
	if (ethertype == ETHER_TYPE_8021Q) {
		...
		if ((pt + ETHER_TYPE_LEN) > (frame + length)) {
			DHD_ERROR(("BCM: %s: short VLAN frame (%d)\n",
			          __FUNCTION__, length));
			return BCME_ERROR;
		}
		ethertype = ntoh16_ua(pt);
...
static int
dhd_cpkt_log_get_pkt_ip_type(osl_t *osh, void *pktbuf,
	uint8 **data_ptr, int *len_ptr, uint8 *prot_ptr)
{
	...
	if (dhd_cpkt_log_get_pkt_ether_type(osh, pktbuf, (uint8 **)&iph,
	...
	if (ippktlen < iplen) {
		DHD_PKT_LOG(("%s: extra frame length ignored\n",
		          __FUNCTION__));
		iplen = ippktlen;
	} else if (ippktlen > iplen) {
		DHD_PKT_LOG(("%s: truncated IP packet (%d)\n",
		           __FUNCTION__, ippktlen - iplen));
		return BCME_ERROR;
	}

	if (iphdrlen < IPV4_OPTIONS_OFFSET || iphdrlen > iplen) {
		DHD_ERROR((" %s: IP-header-len (%d) out of range (%d-%d)\n",
	...
	if ((iph_frag & IPV4_FRAG_MORE) || (iph_frag & IPV4_FRAG_OFFSET_MASK) != 0) {
		DHD_ERROR(("%s: IP fragment not handled\n", __FUNCTION__));
		return BCME_ERROR;
	}
...
dhd_cpkt_log_get_pkt_info(dhd_pktlog_t *pktlog, dhd_pktlog_ring_info_t *pkt_info)
{
	...
	switch (eth_type) {
	case ETHER_TYPE_IP:
		if (dhd_cpkt_log_get_pkt_ip_type(pktlog->dhdp->osh, pkt_info->info.pkt,
			&pdata, &len, &prot) != 0) {
...
static void
dhd_cpkt_log_insert_ts(dhd_cpkt_log_ts_node_t *node, osl_rb_root_t *root)
{
	osl_rb_node_t **new = &root->rb_node, *parent = NULL;
	u64 ts_diff = node->ts_diff;

	while (*new) {
		parent = *new;
		if (ts_diff < CONTAINEROF(parent, dhd_cpkt_log_ts_node_t, rb)->ts_diff)
			new = &parent->rb_left;
		else
			new = &parent->rb_right;
	}

	OSL_RB_LINK_NODE(&node->rb, parent, new);
	OSL_RB_INSERT_COLOR(&node->rb, root);
}
...
dhd_cpkt_log_deinit_tt(dhd_pub_t *dhdp)
{
	...
	while ((n = OSL_RB_FIRST(&pktlog->cpkt_log_tt_rbt))) {
		node = CONTAINEROF(n, dhd_cpkt_log_ts_node_t, rb);
		OSL_RB_ERASE(&node->rb, &pktlog->cpkt_log_tt_rbt);
